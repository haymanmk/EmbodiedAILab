...
## [2026-05-31] ingest + study | Bishop & Bishop *Deep Learning: Foundations and Concepts* (Springer 2024) — Ch 4 + Ch 5

User added the textbook PDF (656 pages) and asked for a tutor walk-through of Chapters 4 + 5 *because Ch 6 references their terminology*. Combined `ingest` (create source page + chapter index) and `study Chapter 4-5` (concept pages + tutor walk-through) in one workflow. Also: installed `poppler` via brew so PDF reading works on this Mac (poppler provides `pdftoppm` for the Read tool's PDF rendering, plus `pdftotext` / `pdfinfo`).

Tutor walk-through delivered as the primary user-facing artifact: ~3000-word piece structured as "the map → Ch 4 narrative → Ch 5 narrative → bridge to Ch 6 → vocabulary cheat sheet." Hit every Ch 4-5 atom Ch 6 leverages: basis functions, single-layer net diagram, MLE→MSE under Gaussian noise, MLE→cross-entropy under Bernoulli/multinomial, sigmoid+softmax as canonical activations, GLM + canonical link, SGD/LMS, weight decay, bias-variance, decision theory, classifier metrics (precision/recall/F/ROC/AUC), discriminant geometry, discriminative-vs-generative taxonomy.

Wiki artifacts created/extended:

- `wiki/sources/Bishop and Bishop - Deep Learning Foundations and Concepts.md` — new source page with extended frontmatter, chapter-level summary, key claims per chapter, notable quotes including the "natural pairing between error function and activation function" §5.4.6 quote (the single most important paragraph for Ch 6+), connections, open questions.
- `wiki/ingestion/Bishop and Bishop - chapters.md` — new chapter index with PDF page ranges (book p → PDF p offset = +19 for front matter), planned concept pages per chapter, status column. Ch 4 + Ch 5 marked `covered`, Ch 6 marked `next`, all others `queued`. Recommended reading orders for foundation thread, diffusion thread, generative-models thread, transformers thread.
- `wiki/concepts/Basis Functions.md` — new concept page. Piano-keys analogy + breakdown. The three standard families (polynomial / Gaussian / sigmoidal). The "linear in $w$, possibly nonlinear in $x$" trick. Quotes Bishop's pivotal sentence motivating deep learning ("Before the advent of deep learning..."). Translation guide: when Ch 6 says "hidden units", "feature map", "learned representations" — read as basis functions.
- `wiki/concepts/Cross-Entropy Loss.md` — new concept page. Surprise-meter / weather-forecaster analogy with breakdown. Binary + multi-class formulas. Origin from MLE on Bernoulli/multinomial. Clean-gradient miracle and the canonical-link pairing rule. Vanishing-gradient warning for wrong pairings. PyTorch `BCEWithLogitsLoss` / `CrossEntropyLoss` numerical-stability idiom. KL-divergence decomposition. Worked 3-class example.
- `wiki/concepts/Logistic Regression.md` — new concept page. Voting-committee + thermometer analogy with breakdown. Sigmoid derived as log-odds → probability formula. MLE = binary cross-entropy. Single-layer network diagram. Linearly-separable weight-blowup gotcha. Parameter-count comparison vs generative Gaussian classifier (linear vs quadratic in $M$). PyTorch idiom with `BCEWithLogitsLoss`.
- `wiki/concepts/Softmax.md` — new concept page. Voting-with-megaphone analogy with breakdown. Translation invariance. Log-odds derivation generalizing the sigmoid case. Denominator as [[Normalizing Constant]]. Softmax regression (multinomial logistic regression). Numerical stability via log-sum-exp. PyTorch `CrossEntropyLoss` takes-logits-not-probabilities gotcha.
- `wiki/concepts/Generalized Linear Models.md` — new concept page. Plumbing-fittings analogy with breakdown. The canonical three pairings (Gaussian/identity/MSE, Bernoulli/sigmoid/BCE, multinomial/softmax/CCE) and the gradient-cancellation derivation. Vanishing-gradient warning for off-canonical pairings (sigmoid+MSE). Exponential-family sketch. PyTorch decision table for the user's future code.
- `wiki/concepts/Linear Regression.md` — new concept page. Graphic-equalizer analogy with breakdown. MLE = MSE derivation under Gaussian noise. Normal equations + [[Moore-Penrose Pseudoinverse]] closed form. Orthogonal-projection geometry. LMS / SGD. L2 regularization closed form. Bias-variance decomposition. NumPy worked example.
- `wiki/concepts/Sigmoid Function.md` — **extended** existing page with: (1) `sources:` frontmatter pointing to Bishop & Bishop; (2) new section "Where it actually comes from: the log-odds → probability formula" deriving sigmoid from Bayes' theorem on log-odds; (3) new practical-pitfall bullet warning against sigmoid+MSE pairing and cross-linking to [[Cross-Entropy Loss]] + [[Generalized Linear Models]]; (4) Related-concepts list expanded with [[Logistic Regression]], [[Softmax]], [[Cross-Entropy Loss]], [[Generalized Linear Models]], [[Basis Functions]] all promoted from red-links to working concept pages; (5) Mentions block populated.
- `index.md` — added Bishop source page, Bishop chapter index, and 5 new concept pages (Basis Functions, Cross-Entropy Loss, Generalized Linear Models, Linear Regression, Logistic Regression, Softmax) with one-line summaries.
- `wiki/syntheses/learning-tracker.md` — added 6 new concept rows + 1 textbook resource row to coverage map; appended session log entry.

Genuine knowledge additions for the user:
1. The **"canonical link" pairing rule** — every output activation has a "right" loss, and pairing correctly gives the clean $(y-t)\phi$ gradient while pairing wrongly causes vanishing-gradient training stalls. This is the single most actionable Ch 5 insight for any future PyTorch code.
2. The sigmoid is not arbitrary — it's the formula that converts a log-odds ratio into a probability. Same for softmax in the multi-class case. This reframes both as *probabilistic objects*, not just convenient squashing functions.
3. Hidden layers in Ch 6+ are **learned basis functions** — the basis-function template never goes away in deep learning; it just gets recursive.

Curriculum positioning: this tutor session also unblocks several queued items. [[Diffusion Crash Course - synthesis]] cites Bishop §16.1-16.3 as its top reading recommendation — now that the textbook is ingested and the foundation chapters covered, the Saturday-morning probability/ELBO block has a clear runway. Also: prior tutor work on [[Sigmoid Function]] is now backed by the canonical Bishop reference; the prior page's "vanishing gradient pitfall" claim now has a textbook-grounded *cause* (off-canonical activation-loss pairing) rather than just being a black-box practical warning.

## [2026-05-31] maintenance | relax AGENTS.md raw-immutability rule; LaTeX cleanup on four diffusion-domain raw files

Per user request and explicit choice between two options: the AGENTS.md hard rule on `raw/` immutability is relaxed. Substance, claims, attributions, and structure of raw files must still stay verbatim; rendering-only fixes (broken LaTeX delimiter pairs, missing function macros, invisible-function-application U+2061 characters, broken alignment environments) are now allowed and must be logged.

- `AGENTS.md` — edited Hard Rules section: "Never modify files in `raw/`" → "Never alter substance of files in `raw/`; rendering-only fixes are allowed and must be logged." Edited the Three Layers section's `raw/` description to match. Schema-evolution note retained.
- `wiki/assets/scripts/clean-clipped-latex.py` — new reusable cleanup script (one-off + future use on new clippings). Fixes: `\left(\right.` and family → proper `\left(`/`\right)` pairs; `\left|\right.` → `\mid`; backslash-stripped operator macros (`sim`/`log`/`exp`/`cos`/`sin`/`tan`/`min`/`max`/`sup`/`inf`/`lim`/`ln`/`arg`/`softmax`) re-prefixed with backslash; invisible-function-application U+2061 characters stripped; ` ;\text{` → ` \quad\text{`; multi-line `$$ ... & ... \\ ... $$` blocks wrapped in `\begin{aligned}...\end{aligned}` so Obsidian's MathJax actually renders the alignment. Only processes `$$...$$` display blocks (not inline math or prose). Idempotent — running twice changes nothing.
- Ran the script on all four `raw/diffusion-model/*.md` files. **Cleaned**: `What are Diffusion Models?.md` (Weng), `Generative Modeling by Estimating Gradients of the Data Distribution.md` (Song). **No change**: both Eric Jang normalizing-flows tutorials — those raws don't have the broken-delimiter problem; their issue is the Obsidian Web Clipper dropped inline `$...$` math entirely (variable definitions missing in prose), which can't be recovered from the raw file alone — only by going back to the source URL. Source pages flag this caveat at the top.

Verified the cleanup on Weng's file by diffing pre/post: forward-kernel equations now read `q(x_t \mid x_{t-1}) = \mathcal{N}(\ldots)` cleanly; aligned derivations in the reparameterization-trick block are wrapped in `\begin{aligned}` so Obsidian renders them as a single multi-line display equation. Backups saved at `/tmp/pre-clean-*.md` for the session.

## [2026-05-31] ingest | Eric Jang Normalizing Flows tutorials (Parts 1 + 2) — auxiliary cluster for diffusion crash course

Ingested both normalizing-flows tutorials by Eric Jang (2018, evjang.com) as part of the weekend diffusion-curriculum build. Not strictly required to understand diffusion, but they fill in the *other* likelihood-based generative-modeling family (change-of-variables + invertibility) and the probability-flow ODE in Yang Song's blog IS a continuous normalizing flow — so the formalism shows up on the diffusion path.

- `wiki/sources/Normalizing Flows Tutorial Part 1 - Eric Jang.md` — new source page with extended frontmatter. Raw-file caveat flagged: Web Clipper dropped most inline `$...$` math. Summary covers the four-move structure (why Gaussians fail → 1D change of variables → multivariate Jacobian-determinant → stacking bijectors into a flow); ~8 key claims including the LeakyReLU bijector, the $O(n^3)$ determinant problem that motivates all NF research, and why sigmoid/tanh are bad bijectors; four notable quotes including the determinants-as-volume-distortion line.
- `wiki/sources/Normalizing Flows Tutorial Part 2 - Eric Jang.md` — new source page. Same raw-file caveat. Summary covers the architecture menu (MAF / IAF / Real-NVP / Parallel WaveNet / BatchNorm bijector) with the consistent "fast forward vs fast inverse vs cheap Jacobian" trade-off framing; ~10 key claims including the autoregressive-models-ARE-flows reinterpretation, MAF/IAF inverse-of-each-other relationship, variable-ordering-permutation gotcha, and NF-as-reversible-computation (RevNets memory trick).
- Both source pages cross-link to the new [[Diffusion Crash Course - synthesis]], to [[Diffusion Models]], and to [[Generative Modeling by Estimating Gradients of the Data Distribution - Yang Song]] (where the probability-flow ODE makes the NF connection load-bearing).

## [2026-05-31] synthesis | Diffusion Crash Course — Weekend Edition

Per user request: build a weekend (~12–16 active hours) curriculum to acquire "the whole picture" of diffusion-models — every named concept seen once, every major derivation walked through at least at intuition level, every modern variation placed on the map. Goal is the index in the user's head, not depth; depth comes from future deep-reads picked from a per-interest matrix at the end.

- `wiki/syntheses/Diffusion Crash Course - synthesis.md` — new curriculum synthesis page. Structure: Premise → "Where you are on day 0" (leveraging existing vault coverage) → "What you'll have on day 2" → **Four-block weekend plan**: Saturday-morning probabilistic ML fundamentals (KL, MLE, ELBO via Jensen, reparameterization trick) → Saturday-afternoon score matching + Langevin → Sunday-morning DDPM derivation → Sunday-afternoon big-picture (DDPM↔score unification, SDE, DDIM, CFG, latent diffusion, U-Net/DiT/ControlNet, inverse problems via Bayes-on-scores, Diffusion Policy preview). Auxiliary cluster on normalizing flows (~1.5h, optional). "Beyond the weekend" depth-dive matrix with six per-interest paths. "Quick reference" section lists four recommended textbooks (Bishop & Bishop 2024 is the top pick) and ten foundational papers in study order. Pacing notes for falling-behind contingencies. "One-shot read" section at the bottom for the time-constrained — a 1-hour version of the curriculum.
- Each block has explicit **Goals** (checkboxes) and **Socratic checks** to gate progression. Readings cite both vault concept pages and external textbooks/papers/blog URLs.
- Cross-linked from [[Diffusion Models]], [[Normalizing Constant]], the two diffusion source pages, the two new normalizing-flows source pages, and [[learning-tracker]].

- `index.md` — added the two new normalizing-flows source pages under Sources; added [[Diffusion Crash Course - synthesis]] under Syntheses.
- `wiki/syntheses/learning-tracker.md` — added a Resource progress row for the curriculum synthesis; appended this session log entry.

Curriculum design rationale: the user explicitly framed this as "the whole picture rather than depth" — so the curriculum prioritizes seeing every named idea once over deriving any one in full. The Socratic checks gate progression on understanding the *concepts*, not on being able to reproduce derivations. Saturday's two blocks are pure scaffolding (probability + score); Sunday's two blocks are the diffusion-specific content. The "Beyond the weekend" matrix is the deliberate exit ramp so the user can choose where to go deep based on actual interest, not curriculum prescription.
